...
# Create a vector of symptoms, with each element corresponding
# to a symptom
words1 <- words_split(merged_df1$symptoms, sep = "; ")
...
head(colnames(merged_df1), 10)
head(colnames(test2), 10)

# Reorganize the columns
test2 <- cbind(test2[ , 1:7], duration = test2[, 87], test2[8:86])
...
# Merge the datasets
full_df1 <- rbind(merged_df1, test2)
...
```{r}
# Load the old training set
train1 <- read.csv("data/train.txt")
# Clean age and symptoms
...
merged_df1 <- full_df1[0:638, ]
test2 <- full_df1[639:838, ]
