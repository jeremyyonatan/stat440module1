...
for( i in c(0:length(words2) ) ){
      new[i] <- str_detect(words2[i], ".*weak|weaK.*")
}

  
for( i in c(0:length(words2) ) ){
      new[i] <- str_detect(words2[i], "diarr.")
}


for( i in c(0:length(words2) ) ){
      new[i] <- str_detect(words2[i], "runny nose|rhinorreah")
}


for( i in c(0:length(words2) ) ){
      new[i] <- str_detect(words2[i], "sputum|expectoration")
}
}
```
...
for( i in c(0:length(words2) ) ){
      new[i] <- str_detect(words2[i], "musc.*|.*musc|discomfort.*|.*discomfort|malaise|myalgia|.*physical|pysical.*")
}
...
for( i in c(0:length(words2) ) ){
      new[i] <- str_detect(words2[i], ".37|37.")
}

for( i in c(0:length(words2) ) ){
      new[i] <- str_detect(words2[i], ".39|39.")
}
...
for( i in c(0:length(words2) ) ){
      new[i] <- str_detect(words2[i], ".*39|39.*|fever")
}
  
for( i in c(0:length(words2) ) ){
      new[i] <- str_detect(words2[i], ".*chest|chest.*")
}

for( i in c(0:length(words2) ) ){
      new[i] <- str_detect(words2[i], ".*pneu|pneu.*|respir.*")
}

  for( i in c(0:length(words2) ) ){
      new[i] <- str_detect(words2[i], ".*cough|cough.*|sneeze")
}

for( i in c(0:length(words2) ) ){
      new[i] <- str_detect(words2[i], ".*dry|dry.*")
}
...
test2 <- cbind(test2[ , 1:7], duration = test2[, 58], test2[8:57])
...
Testing lubridate on confirmed column
```{r}
